<template>
  <div class="post-detail">
    ...
    <div v-else-if="error" class="post-detail__error">
      <p>{{ error }}</p>
      <AppButton @click="navigateToList" variant="outline">게시판으로 돌아가기</AppButton>
    </div>
          ...
          <div class="post-detail__actions">
            ...
            <div v-if="isAuthorOrAdmin" class="post-detail__author-actions">
              <AppButton 
                ...
                size="sm"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="post-detail__icon">
          ...
          <div class="post-detail__meta">
            <div class="post-detail__author">
              ...
              <div class="post-detail__author-info">
                <span class="post-detail__author-name">{{ post.nickname }}</span>
                <span class="post-detail__date">{{ formatDate(post.createdAt) }}</span>
              </div>
            </div>
...
const isAuthorOrAdmin = computed(() => {
  ...
  // 작성자인 경우 수정/삭제 가능
  return authStore.currentUser && post.value.memberId === authStore.currentUser.id
})
...
const deletePost = async () => {
  try {
    ...
    // 오류 응답에 따른 분기 처리
    if (err.response && err.response.status === 401) {
      notificationStore.showError('세션이 만료되었습니다. 다시 로그인해주세요.', { 
        duration: 5000 
      });
      authStore.logout();
      router.push({ name: 'login', query: { redirect: route.fullPath } });
