<template>
  <div class="posts-list">
    <div class="posts-list__container">
      ...
      <div class="posts-list__header">
        <div class="board-tabs" v-if="boardStore.boards.length > 0">
          <button
            ...
            :key="board.id"
            :class="['board-tab', 'glass-btn', { 'active': selectedBoardType === board.name }]"
      ...
      <!-- 마지막 페이지 -->
      <div v-if="!isLoading && !hasMore && posts.length > 0" class="posts-list__end">
        <p>모든 게시글을 불러왔습니다.</p>
      </div>
    </div>
  </div>
...
const loadPosts = async () => {
  ...
  try {
    const response = await PostService.getPosts(page.value, size.value, selectedBoardType.value);

    // API 응답 구조: { posts: [...], maxPage: number }
    if (response.data && response.data.data.posts && response.data.data.posts.length > 0) {
      // 게시글 배열에 추가
      posts.value = [...posts.value, ...response.data.data.posts];

      // 최대 페이지 정보 업데이트
      maxPage.value = response.data.data.maxPage || 1;
