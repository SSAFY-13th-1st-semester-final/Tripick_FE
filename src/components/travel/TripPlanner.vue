<template>
  <div class="trip-plan-container" :class="{ 'modal-mode': isModal }">
    ...
    <div class="container" :class="{ 'container-modal': isModal }">
      <div class="trip-form-container glass-card" :class="{ 'modal-form': isModal }">
        ...
        <div class="trip-form">
          ...
          <div v-if="currentStep === 1" class="trip-step">
            ...
            <div class="form-group">
              ...
              <RegionSelector v-model="selectedRegion" @region-selected="onRegionSelected" />
            </div>
            ...
            <div class="action-buttons" :class="{ 'with-cancel': isModal }">
              <button v-if="isModal" class="glass-btn" @click="$emit('close')">취소</button>
              <button class="glass-btn primary" :disabled="!selectedRegion || !tripTitle.trim()" @click="goToNextStep">다음</button>
            </div>
          </div>
          ...
          <div v-if="currentStep === 2" class="trip-step">
            ...
            <div v-if="dateRange.startDate && dateRange.endDate" class="trip-duration">
            ...
            <div class="action-buttons">
              ...
              <button class="glass-btn primary" :disabled="!isDateValid" @click.prevent="createTripPlan">여행 계획 시작하기</button>
            </div>
          </div>
        </div>
      </div>
...
const isFormValid = computed(() => {
  ...
  return tripTitle.value.trim() !== '' && selectedRegion.value !== null && isDateValid.value;
});

// 메서드
const goToNextStep = () => {
  if (currentStep.value < 2) { // 2단계로 변경
    currentStep.value++;
  }
};
...
const getFormattedDateRange = () => {
  ...
  return `${formatDate(dateRange.value.startDate)} ~ ${formatDate(dateRange.value.endDate)}`;
};
