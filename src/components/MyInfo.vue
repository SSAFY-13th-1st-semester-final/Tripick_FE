<template>
    <div class="p-4 max-w-sm mx-auto bg-white rounded shadow-lg flex flex-col justify-between">
      <div>
        <!-- 상단 내용 영역 -->
        <div>
          <!-- 프로필 이미지 영역 (중앙 정렬) -->
          ...
          <div class="text-center w-full">
            <div class="flex justify-center items-end space-x-4 mb-3">
              <div class="text-xl font-bold">{{ member.name }}</div>
              <div class="text-xs text-gray-400">{{ userRole }}</div>
            </div>

                      <!-- 수정모드일 때만 이메일 입력 폼 표시 -->
          <div v-if="!editMode" class="text-xs text-gray-400 mb-2">
            <span>{{ member.email }}</span> <!-- 이메일 텍스트 표시 -->
          </div>
        ...
        >
          <div v-if="editMode">
            ...
            <div class="relative mb-3 mt-4"> <!-- 여기에 mt-4 추가 -->
              <input
  ...
  const enableEdit = () => {
    editMode.value = true
    dynamicHeight.value = 20 // 예시로 25vh 만큼 늘어남
  }
